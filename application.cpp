...
static void imgui_color_layout(){
...
static void end(GLFWwindow* p_window, const framebuffer_extent& p_extent){
    ImGuiIO& io = ImGui::GetIO();
    io.DisplaySize.x = (float)p_extent.width;
    io.DisplaySize.y = (float)p_extent.height;
    ImGui::Render();
...
int main(){
    ...
    // Creating a sphere
    flecs::entity mesh_entity = scene_registry.entity("Sphere");
    ...
    mesh_entity.set<mesh_renderer>({
        ...
        .color = {0.5f, 0.f, 0.5f, 1.f},
        .textures_path = {"assets/wood.png"}
    });

    flecs::entity mesh_entity2 = scene_registry.entity("Robot");
    ...
    mesh_entity2.set<mesh_renderer>({
        ...
        .textures_path = {"assets/robo-pose/textures/Texture_1K.jpg", "assets/robo-pose/textures/LP_BodyNormalsMap_1K.jpg", "assets/robo-pose/textures/specular.jpeg", "assets/robo-pose/textures/diffuse.jpeg"}
    });
    ...
    // This enables depth testing with OpenGL
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        query_transforms.each([&](flecs::entity p_entity, transform& p_transform, mesh_renderer& p_mesh){
            geometry_renderer.submit(p_entity.id(), &p_transform, p_mesh);
        });
        ...
        begin();
        float progress_fraction = 10.f;
        custom_healthbard(progress_fraction, extent);
